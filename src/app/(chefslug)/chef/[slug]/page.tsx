...
export async function generateStaticParams() {
  try {
    const res = await fetch(API_BASE + "/cooks?limit=50&page=1", { cache: "force-cache" });
    if (!res.ok) return [{ slug: "placeholder" }];
    const json = await res.json();
    const items = json?.data?.cooks ?? json?.data ?? json?.cooks ?? [];
    if (!Array.isArray(items) || items.length === 0) return [{ slug: "placeholder" }];
...
export default async function ChefDetailPage({ params }: { params: { slug: string } }) {
  ...
  const chefUrl = "https://thecookoncall.com/chef/" + params.slug;
  ...
  const personSchema = {
    ...
    url: chefUrl,
    jobTitle: "Home Chef",
    description: chef.bio ?? name + " is a verified home chef in Ahmedabad specialising in " + (cuisines.join(", ") || "home cooking") + ".",
  ...
  const serviceSchema = {
    ...
    "@type": "Service",
    "@id": chefUrl + "#service",
    name: "Home Chef Service by " + name,
    provider: { "@id": chefUrl + "#chef" },
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Chefs in Ahmedabad", item: "https://thecookoncall.com/chef" },
      { "@type": "ListItem", position: 3, name, item: chefUrl },
    ],
  };
