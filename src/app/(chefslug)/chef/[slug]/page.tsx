...
export async function generateStaticParams() {
  try {
    ...
    return (json?.data?.cooks ?? []).map((c: Cook) => ({ slug: String(c.id) }));
  } catch { return [{ slug: "placeholder" }]; }
}
    ...
    <>
      ...
      <main className="min-h-screen bg-[var(--cream-100)]">
        <div className="pt-[88px] pb-16 px-5 max-w-[800px] mx-auto">
          ...
          <div className="bg-white rounded-[20px] p-6 border border-[rgba(212,114,26,0.06)] mb-6">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[var(--orange-500)] to-[var(--orange-400)] flex items-center justify-center text-white text-2xl font-bold mb-4">
              {chef.user?.profile_photo ? (
                <Image src={chef.user.profile_photo} alt={name} width={80} height={80} className="rounded-full object-cover" />
              ) : initials}
            </div>
