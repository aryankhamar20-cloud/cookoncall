...
export async function generateStaticParams() {
  try {
    const res = await fetch(API_BASE + "/cooks?limit=500&page=1", {
      cache: "force-cache",
    });
    if (!res.ok) return [];
...
async function getChef(slug: string): Promise<Cook | null> {
  try {
    const res = await fetch(API_BASE + "/cooks/" + slug, {
      cache: "force-cache",
    });
...
export async function generateMetadata({
  ...
  const description =
    ...
    "Book " +
      name +
      ", a verified home chef in Ahmedabad" +
      (cuisines ? " specialising in " + cuisines : "") +
      ". " +
      (reviews > 0 ? reviews + " reviews. " : "") +
      "Starting at Rs.49 visit fee.";
