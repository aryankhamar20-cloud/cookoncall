...
export async function generateMetadata({
  ...
  const description =
    chef.bio ??
    `Book ${name}, a verified home chef in Ahmedabad${cuisines ? ` specialising in ${cuisines}` : ""}. ${rating}${reviews > 0 ? `${reviews} reviews. ` : ""}Starting at Rs.49 visit fee.`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: chef.user?.profile_photo ?? "/og-default.png", width: 1200, height: 630 }],
    },
    twitter: {
...
export default async function ChefDetailPage({
  ...
  const personSchema = {
    ...
    name,
    jobTitle: "Home Chef",
    description: chef.bio ?? `Verified home chef in Ahmedabad specialising in ${cuisines.join(", ")}`,
        ...
        <div className="pt-[88px] pb-16 px-5 max-w-[800px] mx-auto">
          ...
          <div className="bg-white rounded-[20px] overflow-hidden border border-[rgba(212,114,26,0.06)] mb-6">
            ...
            <div className="pt-14 pb-6 px-6">
              ...
              {cuisines.length > 0 && (
                <p className="text-[0.88rem] text-[var(--text-muted)] mt-1">{cuisines.join(", ")}</p>
              )}
              ...
              {chef.bio && (
                <p className="text-[0.9rem] text-[var(--text-muted)] mt-4 leading-relaxed">{chef.bio}</p>
              )}

              {cuisines.length > 0 && (
                <div className="flex gap-2 flex-wrap mt-4">
                  {cuisines.map((c) => (
                    <span key={c} className="bg-orange-50 text-orange-700 text-xs font-medium px-3 py-1 rounded-full border border-orange-200">
                      {c}
                    </span>
                  ))}
                </div>
